...
//const cover;
export function HexToHsl(arg){
  if(validateHex(arg)){
    return hex2hsl(hex2hex(arg));
  }else{
    throw "this arg is not hex!"
    console.log()
  }
};
export function HslToHex(arg){
  if(validateHsl(arg)){
    return display_hex(hsl2hex(hsl2hsl(arg)));
  }else{
    throw "this arg is not Hsl, hsl(a,b,c)!"
    console.log()
  }
};
export function HslArrToHex(a,b,c){
  let arg=[a,b,c];
  return display_hex(hsl2hex(arg));
}